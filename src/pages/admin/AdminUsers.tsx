...
// Withdrawal interface for user's withdrawal history (with VAT and net amount)
...
export default function AdminUsers() {
  ...
  // Fetch users and their withdrawals (including VAT and net amount)
  const fetchUsers = async () => {
    try {
      const response = await axios.get('/admin/users');
      // For each user, fetch their withdrawals
      const usersWithWithdrawals = await Promise.all(
        response.data.users.map(async (user: User) => {
          try {
            // Fetch withdrawals for this user (API should return VAT and netAmount)
            const res = await axios.get(`/withdrawals?userId=${user._id}`);
            return { ...user, withdrawals: res.data.withdrawals };
          } catch {
            // If error, fallback to empty withdrawals
            return { ...user, withdrawals: [] };
          }
        })
      );
      setUsers(usersWithWithdrawals);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen py-8 bg-gray-50">
      <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        {/* Error Message (always at top, not duplicated in modal) */}
        {errorMsg && (
          <div className="p-3 mb-4 text-center text-red-700 bg-red-100 border border-red-200 rounded">
            {errorMsg}
          </div>
        )}
                  ...
                  <tr key={user._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col gap-1">
                        ...
                        {user.loginAttempts !== undefined && user.maxLoginAttempts !== undefined && !user.isLocked && user.loginAttempts > 0 && (
                          <span className="inline-flex px-2 py-1 text-xs font-semibold text-yellow-800 bg-yellow-100 rounded-full">
                            {user.maxLoginAttempts - user.loginAttempts} attempts left
                          </span>
                        )}
                      </div>
                    </td>
              ...
              <div className="p-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 rounded-lg bg-gray-50">
                    ...
                    {/*
                      Withdrawals VAT and Net Amount Table
                      This table displays each withdrawal for the user, including:
                        - Date of withdrawal
                        - Original amount
                        - VAT (15%) deducted
                    ...
                    */}
                    {selectedUser.withdrawals && selectedUser.withdrawals.length > 0 && (
                      <div className="mt-6">
                        <h4 className="mb-2 font-semibold text-gray-800">Withdrawal VAT & Net Amounts</h4>
                        <div className="overflow-x-auto">
                          <table className="min-w-full text-xs text-left border border-gray-200 rounded-lg">
                            <thead className="bg-gray-100">
                              <tr>
                                ...
                                <th className="px-3 py-2">VAT (15%)</th>
                            ...
                            <tbody>
                              {selectedUser.withdrawals.map((w, idx) => {
                                // Calculate VAT and net amount if not present
                                const vat = w.vat !== undefined ? w.vat : Math.round(w.amount * 0.15);
                                const netAmount = w.netAmount !== undefined ? w.netAmount : w.amount - vat;
                                return (
                                  <tr key={idx}>
                                    <td className="px-3 py-2">{new Date(w.createdAt).toLocaleDateString()}</td>
                                    <td className="px-3 py-2">{w.amount.toLocaleString()} ETB</td>
                                    <td className="px-3 py-2">{vat.toLocaleString()} ETB</td>
                                    <td className="px-3 py-2">{netAmount.toLocaleString()} ETB</td>
                                    <td className="px-3 py-2">{w.status.charAt(0).toUpperCase() + w.status.slice(1)}</td>
                                  </tr>
                                );
                              })}
                            </tbody>
      ...
      {errorMsg && (
        <div className="mt-4 mb-2 text-center">
          <span className="inline-block px-4 py-2 text-sm text-red-700 bg-red-100 rounded-lg">{errorMsg}</span>
        </div>
      )}
      </div>
    </div>
