...
router.get('/stats', async (req, res) => {
  try {
    ...
    const lockedUsers = await User.countDocuments({ lockedUntil: { $gt: new Date() } });
...
router.get('/security-overview', async (req, res) => {
  try {
    const totalUsers = await User.countDocuments();
    const lockedUsers = await User.countDocuments({ lockedUntil: { $gt: new Date() } });
    const usersWithFailedAttempts = await User.countDocuments({ loginAttempts: { $gt: 0 } });
    ...
    res.status(500).json({ message: 'Server error fetching security overview' });
  }
});
